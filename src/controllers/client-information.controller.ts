...
export class ClientInformationController {
	static async getAllClientInformation(c: Context) {
		try {
			const clientInfo =
				await ClientInformationService.getAllClientInformation();
	...
	static async getClientInformationById(c: Context) {
		try {
			...
			const clientInfo =
				await ClientInformationService.getClientInformationById(id);
			if (!clientInfo) {
				return c.json(
					{ success: false, error: "Client information not found" },
					404,
				);
			}
			return c.json({ success: true, data: clientInfo });
	...
	static async createClientInformation(c: Context) {
		try {
			...
			const clientInfo =
				await ClientInformationService.createClientInformation(data);
	...
	static async updateClientInformation(c: Context) {
		try {
			const id = parseInt(c.req.param("id"), 10);
			const data =
				(await c.req.json()) as Partial<CreateClientInformationRequest>;
			const clientInfo = await ClientInformationService.updateClientInformation(
				id,
				data,
			);
			if (!clientInfo) {
	...
	static async deleteClientInformation(c: Context) {
		try {
			const id = parseInt(c.req.param("id"), 10);
			const deleted =
				await ClientInformationService.deleteClientInformation(id);
			if (!deleted) {
				return c.json(
					{ success: false, error: "Client information not found" },
					404,

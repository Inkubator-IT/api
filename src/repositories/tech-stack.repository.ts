...
export class TechStackRepository {
	static async findAll(): Promise<TechStack[]> {
		return (await db
			.select()
			.from(techStack)
			.orderBy(asc(techStack.tech_stack_name))) as unknown as TechStack[];
	}
	...
	static async update(
		...
		if (data.tech_stack_name !== undefined)
			updateData.tech_stack_name = data.tech_stack_name;
		if (data.tech_stack_description !== undefined)
			updateData.tech_stack_description = data.tech_stack_description;

...
export class ClientInformationService {
	...
	static async getClientInformationById(id: number): Promise<ClientInformation | null> {
		if (!id || id <= 0) {
	...
	static async createClientInformation(data: CreateClientInformationRequest): Promise<ClientInformation> {
		...
		if (data.rating_website && (data.rating_website < 1 || data.rating_website > 5)) {
			throw new Error("Rating must be between 1 and 5");
		}
		return await ClientInformationRepository.create(data);
	}

	static async updateClientInformation(id: number, data: Partial<CreateClientInformationRequest>): Promise<ClientInformation | null> {
		if (!id || id <= 0) {
			throw new Error("Invalid client information ID");
		}
